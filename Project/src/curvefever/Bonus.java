...
public class Bonus {
    ...
    public void checkPlayers(List<Player> players) {
        for (Player player : players) {
            ...
            if (Math.sqrt(((playerPosition.x - position.x) * (playerPosition.x - position.x)) +(playerPosition.y - position.y) * (playerPosition.y - position.y)) <= player.getSize() + CurveFeverConsts.BONUS_IMAGE_SIZE / 2) {
